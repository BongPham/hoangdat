...
function get_mans(){
	...
	$sql = "select ten_vi,id,stt,hienthi,id_list,id_cat,noibat,thongdung,id_item,id_sub,banchay,soluong from $where $limit";
	$d->query($sql);
...
function get_items(){
	...
	{
		$id_up = $_REQUEST['noibat'];
		$sql_sp = "SELECT id,noibat FROM table_product_item where id='".$id_up."' ";
		$d->query($sql_sp);
		$cats= $d->result_array();
		$noibat=$cats[0]['noibat'];
		if($noibat==0)
		{
		$sqlUPDATE_ORDER = "UPDATE table_product_item SET noibat =1 WHERE  id = ".$id_up."";
		$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
		}
		else
		{
		$sqlUPDATE_ORDER = "UPDATE table_product_item SET noibat =0  WHERE  id = ".$id_up."";
		$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}	
	}
	...
	$sql = "select ten_vi,id,stt,hienthi,noibat,id_list,id_cat from $where $limit";
	$d->query($sql);
	$items = $d->result_array();
